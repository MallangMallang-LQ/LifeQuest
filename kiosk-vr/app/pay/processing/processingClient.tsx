...
export default function ProcessingClient() {
  ...
  const fail = sp.get('fail'); // 'declined' | 'canceled' | 'timeout' | 'error'

  useEffect(() => {
    const t = setTimeout(() => {
      if (fail) {
        const q = new URLSearchParams({ reason: fail, orderId, amount: String(amount), method });
        router.replace(`/pay/fail?${q.toString()}`);
      } else {
        const paymentKey = `mock_${crypto.randomUUID()}`;
        const q = new URLSearchParams({ paymentKey, orderId, amount: String(amount), method });
        router.replace(`/pay/success?${q.toString()}`);
      }
    }, 1500 + Math.random()*1000);
    return () => clearTimeout(t);
  }, [amount, method, orderId, router, fail]);
