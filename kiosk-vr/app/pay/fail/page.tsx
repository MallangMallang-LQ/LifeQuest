...
const methodLabel = (m?: string) =>
  methodLabels[(m as MethodKey) ?? 'card'] ?? '결제';

const reasonText = (r?: string) => {
  switch (r) {
    case 'declined':
      return { title: '승인 거절', desc: '발급사 또는 결제사에서 승인이 거절되었습니다.' };
    case 'canceled':
      return { title: '사용자 취소', desc: '사용자가 결제를 취소했습니다.' };
    case 'timeout':
      return { title: '시간 초과', desc: '승인 응답 지연으로 결제가 종료되었습니다.' };
    case 'error':
    default:
      return { title: '결제 오류', desc: '일시적인 오류가 발생했습니다. 잠시 후 다시 시도하세요.' };
  }
};
      ...
      <div className="w-full max-w-[640px] bg-white rounded-2xl shadow-lg p-8 text-center">
        ...
        <div className="flex flex-wrap gap-3 justify-center">
          <button
            ...
            className="px-5 py-3 rounded-xl bg-neutral-900 text-white font-extrabold"
          >
            다시 시도
          </button>
          <a
            href={`/checkout/?amount=${amount}`}
            className="px-5 py-3 rounded-xl bg-neutral-100 font-extrabold"
          >
            결제수단 다시 선택
          </a>
