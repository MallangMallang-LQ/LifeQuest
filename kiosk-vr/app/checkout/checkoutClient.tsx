...
type Method = 'kakaopay' | 'tosspay' | 'naverpay' | 'card' | 'samsungpay' | 'applepay';

const QUICK: { id: Method; label: string }[] = [
  { id: 'kakaopay', label: '카카오페이' },
  { id: 'tosspay',  label: '토스페이' },
  { id: 'naverpay', label: '네이버페이' },
];

const CARD: { id: Method; label: string }[] = [
...
export default function CheckoutClient() {
  ...
  const goPay = (method: Method) => {
    const q = new URLSearchParams({ method, amount: String(finalAmount), orderId: crypto.randomUUID() });
      ...
      <div className="w-full max-w-[980px] bg-white rounded-2xl shadow-lg p-6">
        ...
        <section className="mb-8">
          ...
          <div className="flex gap-2">
            ...
            />
            <button onClick={applyCoupon} className="px-5 py-3 rounded-xl bg-neutral-900 text-white font-bold">적용</button>
          </div>
        ...
        <section className="mb-6">
          ...
          <div className="grid grid-cols-3 gap-3">
            {QUICK.map(m => (
              <button key={m.id} onClick={()=>goPay(m.id)} className={clsx('h-16 rounded-xl bg-neutral-50 hover:bg-neutral-100 transition','text-lg font-bold')}>
                {m.label}
              </button>
            ))}
          </div>
        ...
        <section className="mb-2">
          ...
          <div className="grid grid-cols-3 gap-3">
            {CARD.map(m => (
              <button key={m.id} onClick={()=>goPay(m.id)} className="h-16 rounded-xl bg-neutral-50 hover:bg-neutral-100 transition text-lg font-bold">
                {m.label}
              </button>
            ))}
          </div>
        ...
        <div className="mt-6 text-sm text-neutral-500">※ 훈련용 모의결제입니다. 선택 후 <b>결제 중</b> 화면으로 이동하고, 몇 초 뒤 자동으로 완료됩니다.</div>
