...
type MethodId =
  | 'kakaopay' | 'tosspay' | 'naverpay'          // 간편결제
  | 'card' | 'samsungpay' | 'applepay'           // 카드/지갑
  ;
...
export default function Checkout() {
  ...
  const applyCoupon = () => {
    ...
    // 데모 규칙: FIX1000 = 1000원 할인, PCT10 = 10% 할인, FREE = 전액 0원
    if (!code) { setCouponMsg('쿠폰 코드를 입력하세요'); return; }

    if (code === 'FREE') {
      setDiscount(baseAmount);
      setCouponMsg('쿠폰 적용: 전액 할인');
      return;
    }
    if (code === 'FIX1000') {
      setDiscount(Math.min(1000, baseAmount));
      setCouponMsg('쿠폰 적용: 1,000원 할인');
      return;
    }
    if (code === 'PCT10') {
      const d = Math.floor(baseAmount * 0.1);
      setDiscount(Math.min(d, baseAmount));
      setCouponMsg(`쿠폰 적용: ${KR(d)} 할인`);
      return;
    }
          ...
          <div className="grid grid-cols-3 gap-3">
            {QUICK.map(m => (
              <button
                ...
                onClick={()=>goPay(m.id)}
                className={clsx(
                  'h-16 rounded-xl bg-neutral-50 hover:bg-neutral-100 transition',
                  'text-lg font-bold'
                )}
              >
                {m.label}
